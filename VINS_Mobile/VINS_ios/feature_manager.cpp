 ...
 */
//将传入的特征点加入feature这个vector中，并且根据视差判断当前帧是不是关键帧来决定marg掉最老帧还是次新帧
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, Eigen::Vector3d> &image_msg, int &parallax_num)
{
    double parallax_sum = 0;//视差总和
    parallax_num = 0;//满足条件的特征点数量
    ...
    //如果上一次track的点少于20 就添加点 第一个frame_count表示滑窗内的帧数
    //marg最老帧
...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, Eigen::Vector3d> &image_msg, int &parallax_num,const map<int, Eigen::Vector3d> &distorted_image)
{
    double parallax_sum = 0;//视差总和
    parallax_num = 0;//满足条件的特征点数量
    last_track_num = 0;//被跟踪点的个数
    
    map<int , Eigen::Vector3d>::const_iterator distorted_id_pts=distorted_image.begin();
    //[1]遍历图像image中所有的特征点，和已经记录了特征点的容器feature中进行比较
    for (auto &id_pts : image_msg)
    {
        
        FeaturePerFrame f_per_fra(id_pts.second);
        f_per_fra.distorted_point_uv=distorted_id_pts->second;
        //获取特征点的id
        int feature_id = id_pts.first;
        //在feature中查找该feature_id的feature是否存在
        auto it = find_if(feature.begin(), feature.end(), [feature_id](const FeaturePerId &it)
...
void FeatureManager::triangulate(Eigen::Vector3d Ps[], Eigen::Vector3d tic, Eigen::Matrix3d ric, bool is_nonlinear)
{
//    cout<<"outlier_info.size="<<outlier_info.size()<<endl;
    outlier_info.clear();
    for (auto &it_per_id : feature)
    {
        ...
        Eigen::Vector3d p_i = Ps[imu_i], p_j;
        Eigen::Vector3d pts_i = it_per_id.feature_per_frame[0].point, pts_j;
        ...
        //ljl
//        if (it_per_id.estimated_depth < 0.1 || it_per_id.estimated_depth >50)
        {
            std::vector<int> window_id;
            it_per_id.is_outlier = true;
            it_per_id.estimated_depth = INIT_DEPTH;
            outlier_info.push_back(std::make_pair(it_per_id.feature_id, window_id));//emplace_back
        }
    }
}
