...
//在进行PNP求解之前，需要判断当前帧数要大于第l帧，这保证了第l帧直接跳过PNP步骤
                          ...
                          vector<SFMFeature> &sfm_f, map<int, Eigen::Vector3d> &sfm_tracked_points)
{
    ...
    //rotate to cam frame
    Eigen::Matrix3d *c_Rotation = new Eigen::Matrix3d[frame_num];
    Eigen::Vector3d *c_Translation = new Eigen::Vector3d[frame_num];
    ...
    //    首先执行下面的第l帧和最后一帧的三角化，得到共视的特征点
    //    供下面第l+1帧和最后一帧求解PNP，然后利用pnp求解l+1帧到最后一帧的位姿R_initial, P_initial，最后的位姿都保存在Pose中，一次循环，得到l+1，l+2…n-1帧的位姿
...
//    再三角化l帧和i帧（在第l帧和最后一帧之间的帧）之间的3d坐标
    for (int i = l + 1; i < frame_num - 1; i++)
        triangulateTwoFrames(l, Pose[l], i, Pose[i], sfm_f);
    //4: solve pnp l-1; triangulate l-1 ----- l
    //             l-2              l-2 ----- l
//    PNP求解l-1，l-2…0帧和l帧之间的位姿已经三角化相应的特征点坐标
    for (int i = l - 1; i >= 0; i--)
    {
        //solve pnp
