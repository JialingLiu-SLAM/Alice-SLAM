...
:pitch{_pitch},roll{_roll},yaw{_yaw},Tx{_Tx},Ty{_Ty},Tz{_Tz},change_view_manualy{false}
{
    ...
    KF_init = false;
    change_color = false;
    trajectory_color.push_back(cv::Scalar(255, 0, 0));
    indexs.push_back(0);
    look_down = false;
    Ground_idx=0;
}
...
cv::Scalar DrawResult::newColor() {
    ...
    return cv::Scalar( fR*255, fG*255, fB*255 );
}
vector<Eigen::Vector3f> DrawResult::calculate_camera_pose(Eigen::Vector3f camera_center, Eigen::Matrix3f Rc, float length)
{
    vector<Eigen::Vector3f> result;
    vector<Eigen::Vector3f> origin;
    origin.push_back(Eigen::Vector3f(length/2.0,length/2.0,-length*0.7));
    origin.push_back(Eigen::Vector3f(-length/2.0,length/2.0,-length*0.7));
    origin.push_back(Eigen::Vector3f(-length/2.0,-length/2.0,-length*0.7));
    origin.push_back(Eigen::Vector3f(length/2.0,-length/2.0,-length*0.7));
    origin.push_back(Eigen::Vector3f(0,0,0));
    result.clear();
...
Eigen::Vector4f creatPlane(Eigen::Vector3f p1,Eigen::Vector3f p2, Eigen::Vector3f p3)
{
...
//Ax+By+Cz+D = 0;
Eigen::Vector4f DrawResult::findPlane(vector<Eigen::Vector3f> &point_cloud)
{
...
Eigen::Vector3f DrawResult::findGround(vector<Eigen::Vector3f> &point_cloud, vector<Eigen::Vector3f> &inlier_points)
{
    if(!look_down)
        return Eigen::Vector3f(0,0,0);
    int height_range[30];
    double height_sum[30];
    vector<vector<Eigen::Vector3f>> points_clusters;
...
Eigen::Vector3f DrawResult::findZfromXY(Eigen::Vector3f point, Eigen::Vector4f plane)
{
 ...
 */
void DrawResult::computeAR(vector<Eigen::Vector3f> &point_cloud, Eigen::Vector3f &model)
{
     ...
     */
    vector<Eigen::Vector3f> inlier_points;
    int height_range[30];
    double height_sum[30];
    vector<vector<Eigen::Vector3f>> points_clusters;
    points_clusters.resize(30);
...
void DrawResult::drawGround(cv::Mat &result, vector<Eigen::Vector3f> &point_cloud, Eigen::Vector3f P_latest, Eigen::Matrix3f R_latest)
{
    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Eigen::Vector3d(RIC_y,RIC_p,RIC_r)).cast<float>();
    cv::Mat aa(HEIGHT,WIDTH,CV_8UC3,cv::Scalar(0,0,0));
    result = aa;
    ...
    {
        Eigen::Vector3f Pc;
        Pc = (R_latest * RIC).transpose()* (point_cloud[i] - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));//得到相机平面坐标系上的点
...
bool DrawResult::GetFeaturesInArea_1(float x,  float y)
{



         float distx = WIDTH-x;
         float disty = HEIGHT-y;
...
std::vector<Edge> DrawResult::drawGround_another(cv::Mat &result, vector<Eigen::Vector3f> &point_cloud, Eigen::Vector3f P_latest, Eigen::Matrix3f R_latest)
{
    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Eigen::Vector3d(RIC_y,RIC_p,RIC_r)).cast<float>();
    cv::Mat aa(HEIGHT,WIDTH,CV_8UC3,cv::Scalar(0,0,0));
    ...
    for (unsigned int i = 0; i < point_cloud.size(); i++)
    {
        ...
        if(GetFeaturesInArea_1(pts.x,pts.y)){
            points.push_back(Vec2f_(pts.x, pts.y));
        }
...
void DrawResult::drawBox(cv::Mat &result, Eigen::Vector3f corner_0, Eigen::Vector3f corner_x, Eigen::Vector3f corner_y, Eigen::Vector3f corner_z, float size, Eigen::Vector3f P_latest, Eigen::Matrix3f R_latest, bool inAR)
{

    Eigen::Matrix3f RIC;
    RIC = Utility::ypr2R(Eigen::Vector3d(RIC_y,RIC_p,RIC_r)).cast<float>();

    vector<Eigen::Vector3f> boxConers;
    ...
    {
        if (inAR)
            Pc = (R_latest * RIC).transpose()* (it - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));
        else
...
//原版
void DrawResult::drawAR(cv::Mat &result, vector<Eigen::Vector3f> &point_cloud, Eigen::Vector3f P_latest, Eigen::Matrix3f R_latest)
{
    cv::Mat aa(HEIGHT,WIDTH,CV_8UC3,cv::Scalar(0,0,0));
    result = aa;
    Eigen::Matrix3f RIC;
    ...
    Eigen::Vector3f cam_z(0, 0, -1);
    Eigen::Vector3f w_cam_z = R_latest * RIC * cam_z;
    if (acos(w_cam_z.dot(Eigen::Vector3f(0, 0, 1))) * 180.0 / M_PI < 60)
    {
//        printf("look down\n");
        look_down = true;
    }
...
//        printf("not down\n");
    ...
    Eigen::Vector3f groundPlanePoint;
    vector<Eigen::Vector3f> point_inlier;
    groundPlanePoint = findGround(point_cloud, point_inlier);//点最多的一层 ，然后点的3D位置去平均 这个就是AR的位置 世界坐标系
    ...
    if ( locationX != locationX_p or locationY != locationY_p )
    {
        ...
        float cx=240;
        float cy=320;
        Eigen::Vector3f box_center, Pc;
        Eigen::Vector2f box_center_xy, center_input;
        center_input<< xx, yy;
        ...
        for (unsigned int i =0; i< Grounds.size(); i++)
        {
            Pc = (R_latest * RIC).transpose()* (Grounds[i].center - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));
        ...
        if ( dis_min < 120 and Grounds.size()>0 and Grounds[dis_min_idx].moveflag and finger_state == 1)
        {
            ...
            if (RWP(2,2) < 0 )
            {
                Eigen::Vector3f unit_direction, direction;
                unit_direction = RWP*Eigen::Vector3f(0,0,1);
                float length_direction;
                Eigen::Vector3f plane_normal(Grounds[dis_min_idx].initPlane[0], Grounds[dis_min_idx].initPlane[1], Grounds[dis_min_idx].initPlane[2]);
    ...
    ///////////////////////////// rotation response
    {
        ...
        if (X0AR!=X0_p or Y0AR != Y0_p)
        {
            ...
            {
                Eigen::Matrix3f RWC = R_latest * RIC;

                Eigen::Vector3f h_r;
                h_r<<X0_p - X0AR, Y0AR - Y0_p, 0;
                h_r = RWC * h_r;
                Eigen::Vector2f h_hori, h_r_hori;
    ...
    if (longPressFlag)
    {
        ...
        if ( dis_min < 120 and Grounds.size()>0 )
        {
            ...
            Pc = (R_latest * RIC).transpose()* (Grounds[dis_min_idx].coz - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));
    ...
    if (tapFlag and point_inlier.size()>28)
    {
        ...
        Eigen::Vector2f box_center_xy, center_input;
        center_input<< xx, yy;
        Pc = (R_latest * RIC).transpose()* (groundPlanePoint - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));
        box_center_xy.x() = FOCUS_LENGTH_X * Pc.x() / Pc.z()+ PX;//图像坐标
        box_center_xy.y() = FOCUS_LENGTH_Y * Pc.y() / Pc.z()+ PY;

        float distance;
        bool security_flag=true;
        Eigen::Vector2f box_center_his;
        for (unsigned int i =0; i< Grounds.size(); i++)//要求和已有Grounds欧氏距离小于150，否则不加入
        {
            Pc = (R_latest * RIC).transpose()* (Grounds[i].center - 1.0*P_latest  - R_latest * Eigen::Vector3f(TIC_X,TIC_Y,TIC_Z));
        ...
        if ( (box_center_xy - center_input).norm()<150 and security_flag)
        {
            ...
            //ori cox coy coz其实是4个xyz值，后续会选择更好的
            //lengthc其实是正方形的边长 groundPlanePoint是正方形最里面的那个点，要把它变化到正方形外面来
            ori = groundPlanePoint - (linex*lengthc + liney*lengthc) /2.0;// 可以想象成0，0，0  事实上它是世界坐标系，是有深度这些值的
            ...
            grounds_ar_mutex.lock();
            Grounds_send.push(gp);
            grounds_ar_mutex.unlock();
        }
        tapFlag = false;
    }else{
        if(tapFlag)
            cout<<"测试 是不是点数不够："<<point_inlier.size()<<endl;
    }
...
//所有的点都画
/**
std::vector<Edge> DrawResult::drawAR(cv::Mat &result, vector<Eigen::Vector3f> &point_cloud, Eigen::Vector3f P_latest, Eigen::Matrix3f R_latest)
{
...
//        drawGround(result, point_inlier, P_latest, R_latest);
    ...
    //所有的点  都要画
   std::vector<Edge> edges= drawGround_another(result, point_cloud, P_latest, R_latest);
...
void DrawResult::drawBoxVirturCam(cv::Mat &result)
{
    ...
    Eigen::Matrix3f camInWorld_R;
    Eigen::Vector3f Zwc = -camInWorld_T/camInWorld_T.lpNorm<2>();
    Eigen::Vector3f Xwc;
    Xwc  << 1.0, -camInWorld_T.x()/camInWorld_T.y(), 0;
    Xwc = Xwc/Xwc.lpNorm<2>();
    Eigen::Vector3f Ywc = Zwc.cross(Xwc);
...
cv::Point2f DrawResult::World2VirturCam(Eigen::Vector3f xyz, float &depth)
{
     ...
     */
    //camInWorld_R = Utility::ypr2R(Eigen::Vector3f(0, 0, -theta)) * Utility::ypr2R(Eigen::Vector3f(0, phy, 0)) * Utility::ypr2R(Eigen::Vector3f(0, 0, -90));
    Eigen::Vector3f Zwc = -camInWorld_T/camInWorld_T.lpNorm<2>();
    Eigen::Vector3f Xwc;
    Xwc  << 1.0, -camInWorld_T.x()/camInWorld_T.y(), 0;
    Xwc = Xwc/Xwc.lpNorm<2>();
    ...
    Eigen::Vector3f Pc = camInWorld_R.transpose() * (xyz - origin_w - camInWorld_T);
...
void DrawResult::Reprojection(cv::Mat &result, vector<Eigen::Vector3f> &point_cloud, const Eigen::Matrix3f *R_window,const Eigen::Vector3f *T_window, bool box_in_trajectorty)
{
    float depth_marker;
    cv::Mat aa(WIDTH,HEIGHT,CV_8UC3,cv::Scalar(242,242,242));
    result = aa;
    ...
    //draw frame arrow
    {
        ...
        //draw grid
        {
            float dis = 1.0;
            int line_num = 9;
            vector<pair<Eigen::Vector3f, Eigen::Vector3f>> grid_space;
            vector<pair<cv::Point2f, cv::Point2f>> grid_plane;
            Eigen::Vector3f origin_grid;
            origin_grid << - dis*(line_num/2),
            - dis*(line_num/2),
            0;
            for(int i=0; i < line_num; i++)
            {
                pair<Eigen::Vector3f, Eigen::Vector3f> tmp_Pts;
                tmp_Pts.first = origin_grid + Eigen::Vector3f(dis * i, 0, 0);
                tmp_Pts.second = origin_grid + Eigen::Vector3f(dis * i, dis*(line_num - 1), 0);
                grid_space.push_back(tmp_Pts);

                tmp_Pts.first = origin_grid + Eigen::Vector3f(0, dis * i, 0);
                tmp_Pts.second = origin_grid + Eigen::Vector3f(dis*(line_num - 1), dis * i, 0);
                grid_space.push_back(tmp_Pts);
            }
    ...
    for(int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        vector<Eigen::Vector3f> camera_coner_w = calculate_camera_pose(T_window[i], R_window[i], length);
