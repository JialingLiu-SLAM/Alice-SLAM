...
{
    PerspectiveFactor::sqrt_info = FOCUS_LENGTH_X / 1.5 * Eigen::Matrix2d::Identity();
}
...
void vinsPnP::setInit(VINS_RESULT vins_result)
{
    for (int i = 0; i <= PNP_SIZE; i++)
    {
        ...
        if(fabs(Headers[i] - vins_result.header)<0.00001)
...
void vinsPnP::updateFeatures(vector<IMG_MSG_LOCAL> &feature_msg)
{
    ...
    {
        int features_i_len=features[i].size();
        if(features_i_len>0){
            int j = 0;
            for (auto &it : feature_msg)//70个点
            {
//                cout<<"it.id="<<it.id<<endl;
                while(features[i][j].id < it.id)//features  存在id没有获得
                {
                    if(features_i_len<=j){
                        break;
                    }
...
void vinsPnP::new2old()
{
    ...
    solved_index = 0;
    Eigen::Vector3d origin_R0 = Utility::R2ypr(Rs[solved_index]);
    
    Eigen::Vector3d origin_P0 = Ps[solved_index];
    
    Eigen::Vector3d origin_R00 = Utility::R2ypr(Eigen::Quaterniond(para_Pose[solved_index][6],
    ...
    for (int i = 0; i <= PNP_SIZE; i++)
    {
        /*
         ...
         Ps[i] = rot_diff * Eigen::Vector3d(para_Pose[i][0] - para_Pose[solved_index][0],
        ...
        Bas[i] = Eigen::Vector3d(para_Bias[i][0],
                          para_Bias[i][1],
                          para_Bias[i][2]);
        
        Bgs[i] = Eigen::Vector3d(para_Bias[i][3],
                          para_Bias[i][4],
                          para_Bias[i][5]);
        //printf("pnp after solve header %lf %d P: %lf %lf %lf Bgs: %lf %lf %lf\n",Headers[i], find_solved[i], Bas[i].x(), Bas[i].y(), Bas[i].z(), Bgs[i].x(), Bgs[i].y(), Bgs[i].z());
        Eigen::Vector3d R_ypr = Utility::R2ypr(Rs[i]);
    ...
    printf("pnp after solve P: %lf %lf %lf \n",Ps[0].x(), Ps[0].y(), Ps[0].z());
    
}

void vinsPnP::processIMU(double dt, const Eigen::Vector3d &linear_acceleration, const Eigen::Vector3d &angular_velocity)
{
    if (!first_imu)
    {
        first_imu = true;
