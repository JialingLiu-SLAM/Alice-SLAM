...
//  VINS_ios
//
//  Created by 栗大人 on 2017/6/18.
//  Copyright © 2017年 栗大人. All rights reserved.
//

#ifndef imu_factor_pnp_h
...
public:
    IMUFactorPnP() = delete;
    IMUFactorPnP(IntegrationBase* _pre_integration):pre_integration(_pre_integration)
    {
    }
...
//                }
            ...
            if (jacobians[2]) //bias i
            {
                Eigen::Map<Eigen::Matrix<double, 15, 6, Eigen::RowMajor>> jacobian_bias_i(jacobians[2]);
                jacobian_bias_i.setZero();
                jacobian_bias_i.block<3, 3>(O_P, O_BA - O_V - O_R) = -dp_dba;
                jacobian_bias_i.block<3, 3>(O_P, O_BG - O_V - O_R) = -dp_dbg;
                
#if 0
                jacobian_bias_i.block<3, 3>(O_R, O_BG - O_V - O_R) = -dq_dbg;
#else
                Eigen::Quaterniond corrected_delta_q = pre_integration->delta_q * Utility::deltaQ(dq_dbg * (Bgi - pre_integration->linearized_bg));
                jacobian_bias_i.block<3, 3>(O_R, O_BG - O_V - O_R) = -Utility::Qleft(Qj.inverse() * Qi * corrected_delta_q).bottomRightCorner<3, 3>() * dq_dbg;
#endif
                
                jacobian_bias_i.block<3, 3>(O_V, O_BA - O_V - O_R) = -dv_dba;
                jacobian_bias_i.block<3, 3>(O_V, O_BG - O_V - O_R) = -dv_dbg;
                
                jacobian_bias_i.block<3, 3>(O_BA, O_BA - O_V - O_R) = -Eigen::Matrix3d::Identity();
                
                jacobian_bias_i.block<3, 3>(O_BG, O_BG - O_V - O_R) = -Eigen::Matrix3d::Identity();
            ...
            if (jacobians[5])
            {
                ...
                jacobian_bias_j.block<3, 3>(O_BA, O_BA - O_V - O_R) = Eigen::Matrix3d::Identity();
                
                jacobian_bias_j.block<3, 3>(O_BG, O_BG - O_V - O_R) = Eigen::Matrix3d::Identity();
