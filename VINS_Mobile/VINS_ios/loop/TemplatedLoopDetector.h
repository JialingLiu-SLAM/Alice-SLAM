...
public:
  ...
  bool detectLoop(const std::vector<cv::KeyPoint> &keys, 
    ...
    bool detectSimliar_forFeature(const std::vector<cv::KeyPoint> &keys,
      const std::vector<TDescriptor> &descriptors,
...
// --------------------------------------------------------------------------
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::detectSimliar_forFeature(const std::vector<cv::KeyPoint> &keys,
  const std::vector<TDescriptor> &descriptors,
                              DetectionResult &match){
    BowVector bowvec;
    FeatureVector featvec;
    ...
    QueryResults qret;
    m_database->query(bowvec, qret,1, -1);
    qret[0].Id;//这里可以得到最匹配的关键帧
    return false;
