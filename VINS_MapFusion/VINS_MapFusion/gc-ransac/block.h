/* Block_gc.h */
/*
	Template classes Block_gc and DBlock
	Implement adding and deleting items of the same type in blocks.

	If there there are many items then using Block_gc or DBlock
	...
	(3) All Block_gc and DBlock functions are inline, so there are
	    no extra function calls.

	Differences between Block_gc and DBlock:
	(1) DBlock allows both adding and deleting items,
	    whereas Block_gc allows only adding items.
	(2) Block_gc has an additional operation of scanning
	    items added so far (in the order in which they were added).
	(3) Block_gc allows to allocate several consecutive
	...
	Block_gc<MyType> *Block_gc = new Block_gc<MyType>(BLOCK_SIZE);
	...
	// reading items
	for (ptr=Block_gc->ScanFirst(); ptr; ptr=Block_gc->ScanNext())
	{
		printf("%d %d\n", ptr->a, ptr->b);
	}
...
template <class Type> class Block_gc
{
public:
	/* Constructor. Arguments are the Block_gc size and
	   (optionally) the pointer to the function which
	   will be called if allocation failed; the message
	   passed to this function is "Not enough memory!" */
	Block_gc(int size, void (*err_function)(const char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~Block_gc() { while (first) { block *next = first -> next; delete first; first = next; } }

	/* Allocates 'num' consecutive items; returns pointer
	   to the first item. 'num' cannot be greater than the
	   Block_gc size since items must fit in one Block_gc */
	Type *New(int num = 1)
	{
		Type *t;
