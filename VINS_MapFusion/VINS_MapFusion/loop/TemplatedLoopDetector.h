...
{
  /// Exhaustive search 穷尽搜索
  GEOM_EXHAUSTIVE,
  /// Use direct index
  GEOM_DI,
  /// Use a Flann structure 快速最近邻搜索
  GEOM_FLANN,
  /// Do not perform geometrical checking不进行几何检查
  GEOM_NONE
};
...
public:
  ...
  struct Parameters
  {
    ...
    /// Use normalized similarity score? 归一化评分
    bool use_nss;
    /// Alpha threshold
    float alpha;
    /// Min consistent matches to pass the temporal check 局部连续约束
    int k;
    /// Geometrical check 几何检测 得到基础矩阵F
    ...
    /// Distance between entries to be consider a match 匹配的距离
    int dislocal;
...
public:
  ...
  bool detectLoop(const std::vector<cv::KeyPoint> &keys, 
    ...
    bool detectLoop_2(const std::vector<cv::KeyPoint> &keys,
    const std::vector<TDescriptor> &descriptors,
    DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
                          std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec);
    
    int detectLoop_3(const std::vector<cv::KeyPoint> &keys,
    const std::vector<TDescriptor> &descriptors,
    DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
                          std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec);

//    给子地图用的
    bool detectLoop_4(const std::vector<cv::KeyPoint> &keys,
      const std::vector<TDescriptor> &descriptors,
      DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
                            std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec);
  /**
  ...
  void eraseIndex(std::vector<int> &erase_index);
    ...
    bool isGeometricallyConsistent_DI_impl(EntryId old_entry,
...
protected:
    ...
    bool isGeometricallyConsistent_DI_my(EntryId old_entry,
      const std::vector<cv::KeyPoint> &keys,
      const std::vector<TDescriptor> &descriptors,
      const FeatureVector &curvec,
      std::vector<cv::Point2f> &cur_pts,
      ...
      if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
      {
        ...
        if(!qret.empty())
        {
          ...
          if(!islands.empty())
          {
           ...
           /*
              vector<tIsland> islandsReconstruct;
            ...
            for(int i=0; i<islandsReconstruct.size();i++)
            {
                score_average2 += islandsReconstruct[i].best_score;
            }
            score_average2 = score_average2/(double)islandsReconstruct.size();
            for(int i=0; i<islandsReconstruct.size();i++)
            {
                if(islandsReconstruct[i].best_score >= score_average2)
                {
                    islandsReconstruct2.push_back(islandsReconstruct[i]);
                }
            }
...
//                printf("loop use_nss, ns_factor : %lf\n", ns_factor);
//                printf("loop use_nss, m_params.alpha * ns_factor : %lf\n", m_params.alpha * ns_factor);
              }
              else
              {
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::detectLoop_2(const std::vector<cv::KeyPoint> &keys,
const std::vector<TDescriptor> &descriptors,
DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
                  std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec){
    EntryId entry_id = m_database->size();
...
//      FeatureVector featvec;
      ...
      if(m_params.geom_check == GEOM_DI)
        m_database->getVocabulary()->transform(descriptors, m_bowvec, m_featvec,
          m_params.di_levels);
      else
        m_database->getVocabulary()->transform(descriptors, m_bowvec);

      if((int)entry_id <= m_params.dislocal)
      {
        // only add the entry to the database and finish
        m_database->add(m_bowvec, m_featvec);
        match.status = CLOSE_MATCHES_ONLY;
        //printf("entry_id %d <= m_params.dislocal %d\n",entry_id,m_params.dislocal);
      }
      else
      {
        ...
        QueryResults qret;
        m_database->query(m_bowvec, qret, m_params.max_db_results, max_id);

        // update database
        m_database->add(m_bowvec, m_featvec); // returns entry_id
        ...
        if(!qret.empty())
        {
          ...
          if(m_params.use_nss)
          {
            ns_factor = m_database->getVocabulary()->score(m_bowvec, m_last_bowvec);
    //        printf("loop use_nss, ns_factor : %lf\n", ns_factor);
          }
    ...
    const tIsland& island = islandsReconstruct2[0];
                ...
                if(getConsistentEntries() > m_params.k)
                {
                  ...
                  if(m_params.geom_check == GEOM_DI)
                  {
                    ...
                    // all the DI stuff is implicit in the database
                    detection = isGeometricallyConsistent_DI(island.best_entry,
                      keys, descriptors, m_featvec, cur_pts, old_pts);
                  }
                  else // GEOM_NONE, accept the match
                  {
                    detection = true;
    ...
    //                printf("loop use_nss, m_params.alpha * ns_factor : %lf\n", m_params.alpha * ns_factor);
      ...
      if(m_params.use_nss && (int)entry_id + 1 > m_params.dislocal)
      {
        m_last_bowvec = m_bowvec;
      }

      return match.detection();
}

//这里改成 不再是时间一致性 而是共视帧一致性
//template<class TDescriptor, class F>
//int TemplatedLoopDetector<TDescriptor, F>::detectLoop_3(const std::vector<cv::KeyPoint> &keys,
//const std::vector<TDescriptor> &descriptors,
//DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
//                  std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec){
//    EntryId entry_id = m_database->size();
...
//      {
//        //printf("entry_id %d > m_params.dislocal %d\n",entry_id,m_params.dislocal);
//        //max_id=-1 表示所有的都检索，其它表示 检索的id上限
//          int max_id = (int)entry_id - m_params.dislocal;
//
//
//        m_database->query(m_bowvec, qret, m_params.max_db_results, max_id);
...
//              // compute islands
//                vector<tIsland> islands;
//            //当前帧 如果能和回环帧发生匹配，那么和回环帧附件的帧也能发生匹配
//                //这样重复匹配比较浪费时间，随意把他们当做一个孤岛看待
//                //并记录孤岛里面得分最高的
//                //下面会取出孤岛得分最高的那一个
//              computeIslands(qret, islands);
////              // this modifies qret and changes the score order
...
//                const tIsland& island = *std::max_element(islands.begin(), islands.end());
//
//                  best_id=island.best_entry;
////                  match.status = LOOP_DETECTED;
////                //printf("loop chose id:%d socre:%f------------\n",island.best_entry, island.best_score);
////                // check temporal consistency of this island
////                //当前帧附近连续几帧 都能相似性检测到 同一个孤岛附近
////                updateTemporalWindow(island, entry_id);
////
////                // get the best candidate (maybe match)
////                match.match = island.best_entry;
////
////                  //getConsistentEntries() 返回当前帧附件的帧 匹配上的孤岛也在差不多的位置 的次数
////                  //k 应该为4
////                if(getConsistentEntries() > m_params.k)
////                {
////                  //printf("temporal consistent entries:%d > n_params.k:%d\n", getConsistentEntries(),m_params.k);
...
//
              if(m_params.geom_check == GEOM_DI)
              {
                detection = isGeometricallyConsistent_DI(qret[0].Id,
                  keys, descriptors, m_featvec, cur_pts, old_pts);
              }
              else // GEOM_NONE, accept the match
              {
...
//给子地图用
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::detectLoop_4(
  const std::vector<cv::KeyPoint> &keys,
  const std::vector<TDescriptor> &descriptors,
  DetectionResult &match,
  std::vector<cv::Point2f> &cur_pts, std::vector<cv::Point2f> &old_pts,BowVector &m_bowvec, FeatureVector &m_featvec){
    EntryId entry_id = m_database->size();
    match.query = entry_id;
    ...
    QueryResults qret;
    m_database->query(m_bowvec, qret, m_params.max_db_results+m_params.dislocal, -1);
    ...
    if(!qret.empty())
    {
      ...
      {
        ns_factor = m_database->getVocabulary()->score(m_bowvec, m_last_bowvec);
//        cout<<"ns_factor="<<ns_factor<<endl;
//          cout<<bowvec.size()<<bowvec<<endl;
//          cout<<m_last_bowvec.size()<<m_last_bowvec<<endl;
      }
        
      if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)//0.005
      {
        // scores in qret must be divided by ns_factor to obtain the
        // normalized similarity score, but we can
        // speed this up by moving ns_factor to alpha's
...
//        ns_factor=min(ns_factor,0.025);
        ...
        if(!qret.empty())
        {
          ...
          // the best candidate is the one with highest score by now
            ...
            bool isHaveValide=false;
            QueryResults qret_loop;
            int qret_len=qret.size();
            for(int j=0;j<qret_len;j++){
                if(!isHaveValide){
//                    保证返回的不会太长 一直返回到发生回环那一个帧
                    match.matches_vector.push_back(qret[j].Id);
                    
                }
                int curid=qret[j].Id;
//                if((int)entry_id==21 || (int)entry_id==22){
//                    cout<< curid<<" , ";
//                    if(j==10){
//                        cout<<curid<<" , ";
//                    }
...
//                    回环有效
                    if(!isHaveValide ){
                        isHaveValide=true;
                        match.match = qret[j].Id;
                        
                    }
                    
                    qret_loop.push_back(qret[j]);
                }
                
                
            }
//            cout<<endl;
//            cout<<entry_id<<" , "<<max_id<<" , "<<qret_loop.size()<<endl;
//            cout<<endl<<"返回的超过分数的："<<qret_len <<" , "<<match.matches_vector.size()<<endl;

            if(isHaveValide){
                if(qret_loop.size()>m_params.max_db_results){
                    qret_loop.resize(m_params.max_db_results);
                }
                
                match.match = qret_loop[0].Id;
                  ...
                  if(m_params.geom_check == GEOM_DI)
                  {
                    //printf("loop use direct index for geometrical checking\n");
                    // all the DI stuff is implicit in the database
                    detection = isGeometricallyConsistent_DI(qret_loop[0].Id,
                      keys, descriptors, m_featvec, cur_pts, old_pts);
                  }
                  else // GEOM_NONE, accept the match
                  {
    ...
    {
      m_last_bowvec = m_bowvec;
  //      cout<<"last_bowvec"<<entry_id<<endl;
    }
//    return match.detection();
    return isSubMap;
}
...
//            std::cout<<"几何检测失败，"<<cur_output_pts.size()<<" ";
          return false;
        }
    }
    else
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::isGeometricallyConsistent_DI_impl(EntryId old_entry,
                                       const std::vector<cv::KeyPoint> &keys,
                                       const std::vector<TDescriptor> &descriptors,
                                       const FeatureVector &curvec,
                                       std::vector<cv::Point2f> &cur_pts,
                                       std::vector<cv::Point2f> &old_pts){
   return  isGeometricallyConsistent_DI_my(old_entry,keys,descriptors,curvec,cur_pts,old_pts);
}

template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::isGeometricallyConsistent_DI_my(
...
//    cout<<"TemplatedLoopDetector removeLowScores before ";
//    for(auto iter=q.begin(),iter_end=q.end();iter!=iter_end;iter++){
//        cout<<(*iter).Score<<" ";
//    }
//    cout<<endl;
//    cout<<"TemplatedLoopDetector threshold "<<threshold<<endl;
