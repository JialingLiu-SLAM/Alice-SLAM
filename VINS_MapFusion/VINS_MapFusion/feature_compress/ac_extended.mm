...
#define Code_value_bits 20//20 8
...
void output_bit (ac_encoder *ace, int bit)
{
  ace->buffer >>= 1;//最开始为0
  if (bit)
    ace->buffer |= 0x80;//按位 或
  ace->bits_to_go -= 1;
  ace->total_bits += 1;
  if (ace->bits_to_go==0)  {
//      std::cout<<"ace->buffer="<<ace->buffer<<std::endl;
...
{
  long range;
//    std::cout<<"acm->nsym="<<acm->nsym<<std::endl;
  check (sym<0||sym>=acm->nsym, "symbol out of range");
    
//    std::cout<<"sym="<<sym<<std::endl;
//    std::cout<<"ace->high="<<ace->high<<std::endl;
//    std::cout<<"ace->low="<<ace->low<<std::endl;
//    std::cout<<"acm->cfreq[0]="<<acm->cfreq[0]<<std::endl;
//    std::cout<<"acm->cfreq[sym]="<<acm->cfreq[sym]<<std::endl;
//    std::cout<<"acm->cfreq[sym+1]="<<acm->cfreq[sym+1]<<std::endl;
...
//      std::cout<<"ace->low="<<ace->low<<std::endl;
    if (ace->high<Half)  {
      bit_plus_follow (ace, 0);//这里会写入bits
...
//    std::cout<<"ace->bitstream="<<ace->bitstream<<std::endl;
  if (acm->adapt)
...
ac_encode_context (int sym,int d_last ,double *p_last)
{
    ...
    if(d_last!=-1){
        if(sym-1==d_last){
            //分配给sym大概率0-
            p[0]=p_last[0];
            p[1]=p_last[0]+(sym/255.0)*(p_last[1]-p_last[0]);
        }else{
            //还是以前等分的概率
            p[0]=p_last[0]+((sym-1)/255.0)*(p_last[1]-p_last[0]);
            p[1]=p_last[0]+(sym/255.0)*(p_last[1]-p_last[0]);
        }
