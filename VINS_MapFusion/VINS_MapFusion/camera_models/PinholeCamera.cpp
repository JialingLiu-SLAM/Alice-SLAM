...
void
PinholeCamera::Parameters::readFromYamlFile(int m_imageWidth,int m_imageHeight,double k1_global,double k2_global,double p1_global,double p2_global,double FOCUS_LENGTH_X_server,double FOCUS_LENGTH_Y_server,double PX_server,double PY_server)
{
    ...
    m_fx = FOCUS_LENGTH_X_server;
    m_fy = FOCUS_LENGTH_Y_server;
    m_cx = PX_server;
    m_cy = PY_server;
...
//            mx_u=mx_d+d_u(0);
//            my_u=my_d+d_u(1);
//            cout<<"1 mx_u"<<mx_u<<" "<<my_u<<endl;
//            mx_u = mParameters.fx() * mx_u + mParameters.cx();
//            my_u = mParameters.fy() * my_u + mParameters.cy();
//            cout<<"2 mx_u"<<mx_u<<" "<<my_u<<endl;
            
            // Fill matrix with points
//            cv::Mat cv::Mat(1,2,CV_32F);
//
//            cv::Mat.at<float>(0,0)=mx_d;
//            cv::Mat.at<float>(0,1)=my_d;
//
//            cv::Mat K = cv::Mat::eye(3,3,CV_32F);
//            K.at<float>(0,0) = mParameters.fx();
//            K.at<float>(1,1) = mParameters.fy();
//            K.at<float>(0,2) = mParameters.cx();
//            K.at<float>(1,2) = mParameters.cy();
//
//
//            cv::Mat DistCoef(4,1,CV_32F);
//            DistCoef.at<float>(0) = mParameters.k1();
//            DistCoef.at<float>(1) = mParameters.k2();
//            DistCoef.at<float>(2) = mParameters.p1();
//            DistCoef.at<float>(3) = mParameters.p2();
//
//            // Undistort points
//            cv::Mat=cv::Mat.reshape(2);
//            cv::undistortPoints(cv::Mat,cv::Mat,K,DistCoef,cv::Mat(),K);
//            cv::Mat=cv::Mat.reshape(1);
//
//
//            mx_u=cv::Mat.at<float>(0,0);
//            my_u=cv::Mat.at<float>(0,1);
//           cout<<"3 mx_u"<<mx_u<<" "<<my_u<<endl;
//            mx_u = mParameters.fx() * mx_u + mParameters.cx();
//            my_u = mParameters.fy() * my_u + mParameters.cy();
//           cout<<"4 mx_u"<<mx_u<<" "<<my_u<<endl;
        }
    }
...
PinholeCamera::setParameters(const PinholeCamera::Parameters& parameters)
{
    ...
    cout<<"mParameters.fx()"<<mParameters.fx()<<" mParameters.cx()"<<mParameters.cx()<<" m_noDistortion"<<m_noDistortion<<" mParameters.k1()"<<mParameters.k1()<<" mParameters.p1()"<<mParameters.p1()<<endl;
