...
protected:
  ...
  void queryL1(const BowVector &vec, QueryResults &ret, 
    ...
    void queryL1_server(const BowVector &vec, QueryResults &ret,
    int max_results, int max_id) const;
...
//    cout<<"m_voc->getScoringType()="<<m_voc->getScoringType()<<endl;
...
//      cout<<"L1_NORM"<<endl;
    ...
    case L1_NORM_SERVER:{
        queryL1_server(vec, ret, max_results, max_id);
        break;
    }
  }
}
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::queryL1_server(const BowVector &vec,
  QueryResults &ret, int max_results, int max_id) const
{
  BowVector::const_iterator vit;
  typename IFRow::const_iterator rit;
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if((int)entry_id > max_id )
      {
        double value = fabs(qvalue - dvalue) - fabs(qvalue) - fabs(dvalue);
        
        pit = pairs.lower_bound(entry_id);//返回第一个大于等于entry_id的迭代器指针
        if(pit != pairs.end() && !(pairs.key_comp()(entry_id, pit->first)))//比较两个关键字在map中的先后
        {
          pit->second += value;
        }
        else
